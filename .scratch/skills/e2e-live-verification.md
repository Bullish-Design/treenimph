...
devenv shell -- which browsee-lsp   # Remora LSP server
...
devenv shell -- curl -s http://browsee-server:8000/v1/models
```
...
tmux ls 2>&1 | grep "browsee-e2e" && tmux kill-server || echo "Clean"

# Verify no stale .browsee/ state is interfering
ls -la .browsee/ 2>/dev/null   # May or may not exist; that's fine
```
...
| LSP failed to connect | Check browsee.yaml, model server reachability |
...
- [ ] vLLM server reachable (`curl http://browsee-server:8000/v1/models`)
