...
8. [Adapting from Remora Source](#8-adapting-from-browsee-source) — Key differences when running from the example workspace
...
- **browsee** — the Remora package itself (installed editable from `/home/andrew/Documents/Projects/browsee`)
...
- `browsee-lsp` must be available on PATH (installed with the browsee package).
- The Remora model server must be reachable at the URL in `browsee.yaml` (default: `http://browsee-server:8000/v1`), OR set `REMORA_LLM_URL` to override.
...
which browsee-lsp   # Remora LSP server
...
│  ┌────────────────────────────────────────────────────────────────┐  │
│  │  nv2 <file>                                                    │  │
│  │  ├── Neovim with browsee plugin loaded                         │  │
│  │  │   └── browsee.setup({ cmd = "browsee-lsp", ... })            │  │
│  │  └── Remora LSP server (spawned by Neovim)                    │  │
│  │      ├── discovers code nodes in src/                         │  │
│  │      ├── loads bundles from /home/.../browsee/agents/          │  │
│  │      └── runs agent kernel (model requests via browsee.yaml)   │  │
│  └────────────────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────────────────┘
```
...
`python -m e2e.run --scenario startup` can pass without proving a live browsee client attach.
...
devenv shell -- nv2 --headless remora_demo/companion/demo/harness.py \
  "+lua vim.defer_fn(function() local clients=vim.lsp.get_clients({name='browsee'}); print('REMORA_CLIENTS=' .. tostring(#clients)); vim.cmd('qa!') end, 10000)"
```
...
└── browsee.yaml                  # Remora config (bundle_root, model URL, etc.)
```
...
The E2E harness in the Remora source repo (`/home/andrew/Documents/Projects/browsee/e2e/`) was designed for the `remora_demo/project/` demo codebase. When adapting it for this example workspace, the key differences are:

### 8.1 Project Root

| Remora source | Example workspace |
|---------------|-------------------|
| `DEMO_PROJECT = .../remora_demo/project` | `PROJECT_ROOT = .../browsee-example-workspace` |
...
nv2 is the same — it's provided by the shared nixvim import. The Remora plugin auto-setup in `devenv.nix` already points `extraRuntimePaths` at the browsee source tree. No changes needed.
...
1. **LSP didn't start** — check that `browsee-lsp` is on PATH and `browsee.yaml` is valid.
2. **Model server unreachable** — check `REMORA_LLM_URL` / `browsee.yaml` model URL.
...
```bash
tmux kill-session -t browsee-e2e-*
```
